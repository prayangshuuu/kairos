...
from apps.subscriptions.plans import get_plan, get_plan_feature, get_plan_limit
...
def require_feature(feature_code: str, redirect_url: Optional[str] = None):
    ...
    def decorator(view_func: Callable):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not has_feature(request.user, feature_code):
                ...
                return redirect(reverse('subscriptions:pricing') + f"?feature_required={feature_code}")
            return view_func(request, *args, **kwargs)
        return _wrapped_view
    return decorator


def check_limit(limit_code: str, get_current_count: Callable, redirect_url: Optional[str] = None):
