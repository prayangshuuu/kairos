...
def test_overlapping_bookings_raise_integrity_error(host1, event_type):
    ...
    with pytest.raises(IntegrityError):
        Booking.objects.create(
            host=host1,
            event_type=event_type,
            start_at=datetime.datetime(2026, 1, 1, 10, 15, tzinfo=datetime.UTC),
            end_at=datetime.datetime(2026, 1, 1, 10, 45, tzinfo=datetime.UTC),
            status="confirmed",
...
def test_touching_boundaries_succeed(host1, event_type):
    ...
    b2 = Booking.objects.create(
        host=host1,
        event_type=event_type,
        start_at=datetime.datetime(2026, 1, 1, 10, 30, tzinfo=datetime.UTC),
...
def test_buffer_blocks_subsequent_booking(host1, event_type_with_buffer):
    ...
    with pytest.raises(IntegrityError):
        Booking.objects.create(
            host=host1,
            event_type=event_type_with_buffer,
            start_at=datetime.datetime(2026, 1, 1, 10, 40, tzinfo=datetime.UTC),
            end_at=datetime.datetime(2026, 1, 1, 11, 10, tzinfo=datetime.UTC),
            status="confirmed",
            invitee_name="Invitee 2",
            invitee_email="invitee2@example.com",
            invitee_timezone="UTC",
        ...
        try:
            with transaction.atomic():
                Booking.objects.create(
                    ...
                    end_at=datetime.datetime(2026, 1, 1, 12, 30, tzinfo=datetime.UTC),
                    status="confirmed",
