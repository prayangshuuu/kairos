...
def anonymize_user(user: User):
    ...
    with transaction.atomic():
        ...
        future_bookings = Booking.objects.filter(host=user, start_at__gte=now, status=Booking.StatusChoices.CONFIRMED)
        for booking in future_bookings:
            try:
                cancel_booking(booking=booking, cancelled_by="host", reason="Host account deactivated", now=now)
        ...
        for booking in past_bookings:
            ...
            booking.answers = {}
            booking.save(update_fields=['invitee_name', 'invitee_email', 'invitee_notes', 'answers', 'updated_at'])
