...
def test_booking_form_validation(mock_is_slot_available, client):
    ...
    event = EventType.objects.create(owner=user, slug="test", title="Test Event", is_active=True, duration_minutes=30)
    ...
    signer = Signer()
    old_timestamp = (datetime.now(timezone.utc) - timedelta(seconds=10)).timestamp()
    token = signer.sign(str(old_timestamp))
    
    valid_slot = (datetime.now(timezone.utc) + timedelta(days=1)).isoformat()
