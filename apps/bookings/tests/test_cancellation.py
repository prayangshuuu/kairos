...
from apps.bookings.services import create_booking, cancel_booking, SlotUnavailable, AlreadyCancelled, CancellationNotAllowed
...
def test_cancelling_frees_the_slot(mock_is_slot_available):
    ...
    b1 = create_booking(
        event_type=event, start_at=start_at, invitee_name="First", invitee_email="1@test.com",
    ...
    with pytest.raises(SlotUnavailable):
        ...
        create_booking(
            event_type=event, start_at=start_at, invitee_name="Second", invitee_email="2@test.com",
            invitee_timezone="UTC", answers={}, now=now
        )
...
def test_cancelling_twice_raises_already_cancelled():
    ...
    b = Booking.objects.create(
        event_type=event, host=user, start_at=datetime.now(timezone.utc) + timedelta(days=1),
...
def test_cancellation_permissions():
    ...
    event = EventType.objects.create(
        owner=user, schedule=schedule, title="Test", slug="test",
    ...
    with pytest.raises(CancellationNotAllowed):
        cancel_booking(booking=b, cancelled_by="invitee", now=datetime.now(timezone.utc))
        
    # Host CAN cancel
    cancel_booking(booking=b, cancelled_by="host", now=datetime.now(timezone.utc))
    assert b.status == Booking.StatusChoices.CANCELLED
...
def test_cancellation_cutoff():
    ...
    b = Booking.objects.create(
        event_type=event, host=user, start_at=datetime.now(timezone.utc) + timedelta(hours=20),
        end_at=datetime.now(timezone.utc) + timedelta(hours=20, minutes=30),
        status=Booking.StatusChoices.CONFIRMED,
        invitee_email="test@test.com"
    )
