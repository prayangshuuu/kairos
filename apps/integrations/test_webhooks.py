...
from apps.integrations.models import CalendarConnection, SelectedCalendar, WatchChannel, BusyBlock
...
def connection(user):
    return CalendarConnection.objects.create(
        user=user,
        provider='google',
        external_account_id='test-google',
        is_active=True
    )
...
def test_incremental_sync_410_discards_token_and_resyncs(selected_calendar):
    ...
    with patch('apps.integrations.google.client.GoogleCalendarClient') as MockClient:
        ...
        mock_instance.service.events.return_value.list.return_value.execute.side_effect = HttpError(error_resp, b'Gone')
