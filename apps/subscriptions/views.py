...
class PricingView(View):
    ...
    def get(self, request):
        ...
        if not currency:
            ...
            currency = (
                "BDT" if request.META.get("HTTP_ACCEPT_LANGUAGE", "").find("bn") != -1 else "USD"
            )
...
class StripeBillingCheckoutView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        try:
            checkout_url = create_stripe_billing_checkout_session(
...
class StartPaystationSubscriptionView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        start_bdt_paystation_subscription(request.user)
...
class CancelSubscriptionView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        record_subscription_event(
            sub, "user_requested_cancellation", sub.status, payload={"reason": reason}
        )

        messages.info(
            request,
            "Your subscription cancellation request has been recorded. You will retain Pro features until the end of your current period.",
        )
        return redirect("subscriptions:billing_settings")
