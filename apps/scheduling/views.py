            ...
            try:
                ...
                for i, q in enumerate(questions):
                    ...
                    if q.get('field_type') in ['select', 'multiselect', 'radio']:
                        if len(options) < 2:
                            return HttpResponse("Questions with options must have at least two options.", status=400)
        ...
        if questions_json:
            try:
                ...
                for i, q in enumerate(questions):
                    ...
                    if q.get('id'):
                        bq = BookingQuestion.objects.filter(id=q['id'], event_type=self.object).first()
                        if bq:
...
class EventTypeDeleteView(LoginRequiredMixin, View):
    def post(self, request, slug):
        ...
        try:
            ...
            messages.error(request, "Cannot delete event type with existing bookings. Please deactivate it instead.")
        return redirect('scheduling:eventtype_list')
