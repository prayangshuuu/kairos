...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT 1 FROM pg_constraint WHERE conname = 'no_overlapping_bookings_per_host';")
                if cursor.fetchone():
        ...
        if getattr(settings, 'EMAIL_HOST', None):
            self.stdout.write(self.style.SUCCESS(f"✓ Email backend configured ({settings.EMAIL_HOST})"))
        else:
            self.stdout.write(self.style.WARNING("! Email backend NOT configured (using console/dummy?)"))
        ...
        if users_without_schedule == 0:
            ...
            self.stdout.write(self.style.WARNING(f"! {users_without_schedule} user(s) lack a schedule"))
