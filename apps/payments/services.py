...
PAYSTATION_MIN_PAYOUT_THRESHOLD_CENTS = int(
    getattr(settings, "PAYSTATION_MIN_PAYOUT_THRESHOLD_CENTS", 100000)
)
...
def compute_paystation_service_fee(amount_cents: int) -> int:
    ...
    fee_dec = (amount_dec * PAYSTATION_SERVICE_FEE_PERCENT / Decimal("100")).quantize(
...
def create_payment_for_booking(
    *, booking: Booking, payment_account: PaymentAccount | None = None
) -> Payment:
    ...
    if not payment_account:
        payment_account = PaymentAccount.objects.filter(
            user=event_type.owner, provider=provider.name, is_active=True
        ).first()
...
def confirm_payment(*, payment_uid) -> Payment:
    ...
    if payment.status != Payment.STATUS_PENDING:
        logger.warning(f"Attempted to confirm payment {payment.uid} in status {payment.status}.")
        return payment
...
def handle_refund(*, payment: Payment, amount_cents: int | None = None):
    ...
    """
    from apps.payments.providers import PayStationProvider, StripeConnectProvider

    if amount_cents is None:
        amount_cents = payment.amount_cents

    provider = PayStationProvider() if payment.provider == "paystation" else StripeConnectProvider()
    ...
    with transaction.atomic():
        ...
        payment.refund_amount_cents = (
            payment.refund_amount_cents or 0
        ) + result.amount_refunded_cents
        ...
        if payment.provider == "paystation" and result.amount_refunded_cents > 0:
            ...
            fee_reversal_dec = (orig_fee_dec * amount_dec / total_dec).quantize(
                Decimal("1"), rounding=ROUND_HALF_UP
            )
...
@transaction.atomic
def generate_payout(*, host, period_start, period_end, notes: str = "") -> Any | None:
    """
    ...
    current_balance = (
        HostLedger.objects.filter(host=host).aggregate(Sum("amount_cents"))["amount_cents__sum"]
        or 0
    )
    ...
    entries = HostLedger.objects.filter(host=host, created_at__lte=period_end, payout__isnull=True)
    ...
    gross_cents = (
        entries.filter(entry_type="charge").aggregate(Sum("amount_cents"))["amount_cents__sum"] or 0
    )
    fees_cents = abs(
        entries.filter(entry_type="service_fee").aggregate(Sum("amount_cents"))["amount_cents__sum"]
        or 0
    )
