...
def test_saving_weekly_grid_atomically(client):
    ...
    # Initial rules
    AvailabilityRule.objects.create(schedule=schedule, weekday=0, start_time="09:00", end_time="17:00")
    
    url = reverse("scheduling:schedule_update_rules", kwargs={"pk": schedule.id})
    
    # 1. Valid save replaces rules
    valid_grid = [
        {"dayIndex": 0, "dayName": "Monday", "enabled": True, "ranges": [{"start": "10:00", "end": "14:00"}]},
        {"dayIndex": 1, "dayName": "Tuesday", "enabled": False, "ranges": []},
    ]
    response = client.post(url, {"grid_json": json.dumps(valid_grid)})
    assert response.status_code == 302 or response.status_code == 200 # Redirects or returns 200 for HTMX
    ...
    invalid_grid = [
        {"dayIndex": 0, "dayName": "Monday", "enabled": True, "ranges": [
