...
from apps.scheduling.models import Schedule, EventType
from apps.bookings.models import Booking
from apps.bookings.services import create_booking, approve_booking, reject_booking, InvalidTransition
from apps.bookings.tokens import make_approve_token, make_reject_token
...
def test_approving_booking(mock_is_slot_available):
    ...
    event = EventType.objects.create(owner=user, schedule=schedule, title="Test", slug="test", duration_minutes=30, requires_confirmation=True)
...
def test_auto_reject_expired_pending_bookings():
    ...
    user = User.objects.create_user(email="host_auto@example.com", password="password", slug="hostauto")
    schedule = Schedule.objects.create(user=user, name="Default", timezone="UTC")
    event = EventType.objects.create(owner=user, schedule=schedule, title="Test", slug="test", duration_minutes=30, requires_confirmation=True, confirmation_deadline_hours=24)
    ...
    with patch('apps.bookings.services.is_slot_available', return_value=True):
        b_past = create_booking(event_type=event, start_at=past_start, invitee_name="P", invitee_email="p@t.com", invitee_timezone="UTC", answers={}, now=past_start - timedelta(days=2))
        b_future = create_booking(event_type=event, start_at=future_start, invitee_name="F", invitee_email="f@t.com", invitee_timezone="UTC", answers={}, now=now)
        
        # Another booking created far in the past, but for the future. The deadline has passed.
        b_deadline = create_booking(event_type=event, start_at=future_start + timedelta(days=1), invitee_name="D", invitee_email="d@t.com", invitee_timezone="UTC", answers={}, now=now - timedelta(days=2))
    ...
    assert b_deadline.cancellation_reason == "Auto-rejected because the host did not respond in time."
