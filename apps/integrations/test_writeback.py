...
from apps.bookings.models import Booking, BookingReference, Attendee
...
def test_create_event_409_is_success(MockClient, booking, selected_calendar):
    ...
    mock_events.insert.return_value.execute.side_effect = HttpError(resp=mock_resp, content=b'Conflict')
...
def test_delete_event(MockClient, booking, selected_calendar):
    ref = BookingReference.objects.create(
        booking=booking, connection=selected_calendar.connection,
        external_calendar_id='cal_id', external_event_id='evt_id', kind='calendar_event'
    )
...
def test_delete_event_404_success(MockClient, booking, selected_calendar):
    ...
    mock_resp = MagicMock()
    mock_resp.status = 404
    mock_events.delete.return_value.execute.side_effect = HttpError(resp=mock_resp, content=b'Not Found')
