...
from apps.scheduling.models import Schedule, AvailabilityRule, DateOverride, EventType
...
@pytest.fixture
def schedule(owner):
    return Schedule.objects.create(user=owner, name="Test Schedule", timezone="Asia/Dhaka", is_default=True)
...
def test_plain_dhaka_schedule(schedule):
    ...
    # Mon-Fri 09:00-17:00
    for day in range(5):
        AvailabilityRule.objects.create(schedule=schedule, weekday=day, start_time=time(9, 0), end_time=time(17, 0))
    ...
    assert start == datetime(2026, 10, 1, 3, 0, tzinfo=timezone.utc)
    assert end == datetime(2026, 10, 1, 11, 0, tzinfo=timezone.utc)
...
def test_new_york_dst_spring_forward(owner):
    ...
    AvailabilityRule.objects.create(schedule=schedule, weekday=6, start_time=time(9, 0), end_time=time(17, 0))
    ...
    assert start == datetime(2026, 3, 8, 13, 0, tzinfo=timezone.utc)
    assert end == datetime(2026, 3, 8, 21, 0, tzinfo=timezone.utc)
...
def test_new_york_dst_spring_forward_7_hours(owner):
    schedule = Schedule.objects.create(user=owner, name="NY", timezone="America/New_York")
    AvailabilityRule.objects.create(schedule=schedule, weekday=6, start_time=time(0, 0), end_time=time(8, 0))
...
def test_new_york_nonexistent_time(owner):
    ...
    AvailabilityRule.objects.create(schedule=schedule, weekday=6, start_time=time(2, 30), end_time=time(4, 0))
    ...
    assert start == datetime(2026, 3, 8, 7, 0, tzinfo=timezone.utc)
    # 04:00 EDT is 08:00 UTC
    assert end == datetime(2026, 3, 8, 8, 0, tzinfo=timezone.utc)
...
def test_kathmandu_offset(owner):
    ...
    AvailabilityRule.objects.create(schedule=schedule, weekday=0, start_time=time(10, 0), end_time=time(11, 0))
    ...
    # 10:00 KTM -> 04:15 UTC
    assert start == datetime(2026, 1, 5, 4, 15, tzinfo=timezone.utc)
...
def test_lord_howe_dst(owner):
    ...
    AvailabilityRule.objects.create(schedule=schedule, weekday=6, start_time=time(0, 0), end_time=time(4, 0))
...
def test_chatham_extreme(owner):
    ...
    AvailabilityRule.objects.create(schedule=schedule, weekday=6, start_time=time(12, 0), end_time=time(13, 0))
...
def test_date_override_unavailable(schedule):
    AvailabilityRule.objects.create(schedule=schedule, weekday=3, start_time=time(9, 0), end_time=time(17, 0))
...
def test_date_override_custom_hours(schedule):
    ...
    DateOverride.objects.create(schedule=schedule, date=d, start_time=time(10, 0), end_time=time(12, 0))
    DateOverride.objects.create(schedule=schedule, date=d, start_time=time(14, 0), end_time=time(16, 0))
...
def test_split_shifts(schedule):
    d = date(2026, 10, 1) # Thursday
    AvailabilityRule.objects.create(schedule=schedule, weekday=3, start_time=time(9, 0), end_time=time(12, 0))
    AvailabilityRule.objects.create(schedule=schedule, weekday=3, start_time=time(14, 0), end_time=time(17, 0))
