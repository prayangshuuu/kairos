...
from apps.bookings.tasks import create_conference_link, process_booking_confirmation, send_booking_confirmation_emails

pytestmark = pytest.mark.django_db

@pytest.fixture
def booking(user):
    event_type = EventType.objects.create(owner=user, title="Test Event", location_type="google_meet", duration_minutes=30)
    now = timezone.now()
...
@pytest.fixture
def connection(user):
    conn = CalendarConnection.objects.create(user=user, provider='google', external_account_id='test1', external_account_email='test1@google.com')
...
def test_google_meet_idempotency(booking, connection):
    with patch('apps.integrations.google.client.GoogleCalendarClient') as MockClient:
        ...
        # First call succeeds
        mock_instance.service.events.return_value.get.return_value.execute.side_effect = Exception("404")
        ...
        mock_instance.service.events().insert.side_effect = HttpError(Response({'status': 409}), b'Conflict')
...
def test_notifications_chain_ordering(booking, connection):
    # Ensure chain works
    with patch('apps.bookings.tasks.send_booking_confirmation_emails') as mock_email:
