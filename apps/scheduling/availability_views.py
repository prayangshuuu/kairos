...
from .models import Schedule, AvailabilityRule, DateOverride, EventType
    ...
    def get_context_data(self, **kwargs):
        ...
        for i, name in enumerate(weekdays):
            ...
            grid.append({
                ...
                'ranges': [{'start': r.start_time.strftime('%H:%M'), 'end': r.end_time.strftime('%H:%M')} for r in day_rules] if day_rules else [{'start': '09:00', 'end': '17:00'}]
            })
            
        context['grid_json'] = json.dumps(grid)
        context['overrides'] = self.object.overrides.filter(date__gte=timezone.now().date()).order_by('date')
        
        import zoneinfo
        context['timezones'] = sorted(list(zoneinfo.available_timezones()))
        return context
...
class ScheduleDeleteView(LoginRequiredMixin, View):
    def post(self, request, pk):
        ...
        try:
            ...
            if event_types:
                messages.error(request, f"Cannot delete this schedule. It is used by Event Types: {', '.join(event_types)}.")
            else:
    ...
    def post(self, request, pk):
        ...
        for day in grid:
            ...
            for r in ranges:
                ...
                if end <= start:
                    errors.append(f"{day['dayName']}: End time ({end}) must be after start time ({start}).")
                    continue
            ...
            if start_date_str and end_date_str:
                try:
                    ...
                    with transaction.atomic():
                        ...
                        DateOverride.objects.filter(schedule=schedule, date__range=(s_date, e_date)).delete()
