...
from apps.integrations.tasks import send_disconnection_email
...
def test_invalid_grant_deactivates_and_emails(connection):
    ...
    with (
        patch.object(Credentials, "refresh", autospec=True) as mock_refresh,
        ...
        try:
            with patch.object(Credentials, "refresh", autospec=True) as mock_refresh:
                ...
                def fake_refresh(self_creds, request):
                    ...
                    self_creds.expiry = (timezone.now() + datetime.timedelta(hours=1)).replace(
                        tzinfo=None
                    )
