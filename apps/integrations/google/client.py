            ...
            try:
                ...
                if e.resp.status in [429, 500, 502, 503, 504]:
                    if attempt == max_attempts:
                        logger.error(
                            f"Google API transient error max retries reached for connection_id={self.connection.id}. Status: {e.resp.status}"
                        )
                    ...
                    logger.warning(
                        f"Google API transient error {e.resp.status} for connection_id={self.connection.id}. Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                    ...
                    logger.error(
                        f"Google API client error {e.resp.status} for connection_id={self.connection.id}. Body: {e.content}"
                    )
                    raise TransientGoogleApiError(f"API Error {e.resp.status}: {e.reason}")
                else:
                ...
                if attempt == max_attempts:
                    logger.error(
                        f"Google API network/timeout error max retries reached for connection_id={self.connection.id}. Error: {e}"
                    )
                    raise TransientGoogleApiError(f"Network error: {e}")

                delay = base_delay * (2 ** (attempt - 1))
                logger.warning(
                    f"Google API network error for connection_id={self.connection.id}. Retrying in {delay}s... Error: {e}"
                )
                time.sleep(delay)
