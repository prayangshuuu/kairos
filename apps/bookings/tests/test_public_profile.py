...
def test_public_profile_privacy_and_caching(client):
    ...
    EventType.objects.create(owner=user, slug="visible", title="Visible Event", is_hidden=False, is_active=True)
    EventType.objects.create(owner=user, slug="hidden", title="Hidden Event", is_hidden=True, is_active=True)
    ...
    assert "Set-Cookie" not in response.headers or "sessionid" not in response.headers.get("Set-Cookie", "")
