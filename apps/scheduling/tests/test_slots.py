...
@pytest.fixture
def dhaka_owner():
    return User.objects.create_user(email="dhaka@example.com", password="password", timezone="Asia/Dhaka")
...
def schedule(owner):
    ...
    # Mon-Fri 09:00 - 17:00
    for i in range(5):
        AvailabilityRule.objects.create(schedule=s, weekday=i, start_time=time(9, 0), end_time=time(17, 0))
    return s
...
def test_dhaka_host_grouping(dhaka_owner):
    s = Schedule.objects.create(user=dhaka_owner, name="Dhaka", timezone="Asia/Dhaka", is_default=True)
    # Available Mon-Fri 09:00 - 17:00
    for i in range(5):
        AvailabilityRule.objects.create(schedule=s, weekday=i, start_time=time(9, 0), end_time=time(17, 0))
        
    et = EventType.objects.create(
        owner=dhaka_owner, slug="dhk", title="Dhk", duration_minutes=30, schedule=s, max_bookings_per_day=1
    )
    ...
    jan1_slots = [sl for sl in slots if sl.astimezone(ZoneInfo("Asia/Dhaka")).date() == date(2026, 1, 1)]
    jan2_slots = [sl for sl in slots if sl.astimezone(ZoneInfo("Asia/Dhaka")).date() == date(2026, 1, 2)]
...
def test_split_shift(owner):
    ...
    AvailabilityRule.objects.create(schedule=s, weekday=3, start_time=time(9, 0), end_time=time(12, 0))
    AvailabilityRule.objects.create(schedule=s, weekday=3, start_time=time(14, 0), end_time=time(17, 0))
