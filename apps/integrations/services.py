...
from apps.integrations.models import CalendarConnection, BusyBlock
...
def check_live_conflict(user, start: datetime, end: datetime) -> bool:
    ...
    connections = CalendarConnection.objects.filter(user=user, is_active=True).prefetch_related(
        Prefetch('calendars', queryset=BusyBlock.calendar.field.related_model.objects.filter(is_busy_source=True))
    )
