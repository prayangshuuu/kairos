...
class KairosExceptionHandlerMiddleware:
    ...
    def process_exception(self, request, exception):
        if isinstance(exception, KairosError):
            logger.warning("KairosError raised", extra={
            ...
            if request.headers.get("HX-Request"):
                ...
                response["HX-Trigger"] = '{"showToast": {"message": "' + str(exception) + '", "type": "error"}}'
                return response
                
            if exception.status_code == 404:
                return render(request, "errors/404.html", status=404)
            return render(request, "errors/400.html", {"message": str(exception)}, status=exception.status_code)
