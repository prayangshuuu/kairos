...
class BookingForm(forms.Form):
    invitee_name = forms.CharField(max_length=150, required=True, label="Name", widget=forms.TextInput(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}))
    invitee_email = forms.EmailField(required=True, label="Email", widget=forms.EmailInput(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}))
    guest_emails = forms.CharField(required=False, widget=forms.HiddenInput())
    invitee_notes = forms.CharField(
        required=False, 
        widget=forms.Textarea(attrs={'rows': 3, 'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}), 
        ...
        if self.event_type:
            ...
            for q in self.event_type.questions.all().order_by('order'):
                ...
                if q.field_type == 'text':
                    field = forms.CharField(**f_kwargs, widget=forms.TextInput(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}))
                elif q.field_type == 'textarea':
                    f_kwargs['widget'] = forms.Textarea(attrs={'rows': 3, 'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'})
                    ...
                    f_kwargs['widget'] = forms.Select(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'})
                    field = forms.ChoiceField(**f_kwargs)
                elif q.field_type == 'radio':
                    choices = [(opt, opt) for opt in q.options] if q.options else []
                    f_kwargs['choices'] = choices
                    f_kwargs['widget'] = forms.RadioSelect(attrs={'class': 'h-4 w-4 text-primary-600 border-surface-300 focus:ring-primary-500'})
                    field = forms.ChoiceField(**f_kwargs)
                    field.is_radio_or_checkbox_multiple = True
                elif q.field_type == 'multiselect':
                    choices = [(opt, opt) for opt in q.options] if q.options else []
                    f_kwargs['choices'] = choices
                    f_kwargs['widget'] = forms.CheckboxSelectMultiple(attrs={'class': 'h-4 w-4 text-primary-600 border-surface-300 focus:ring-primary-500'})
                    field = forms.MultipleChoiceField(**f_kwargs)
                    field.is_radio_or_checkbox_multiple = True
                elif q.field_type == 'checkbox':
                    field = forms.BooleanField(**f_kwargs, widget=forms.CheckboxInput(attrs={'class': 'h-4 w-4 text-primary-600 border-surface-300 focus:ring-primary-500 rounded'}))
                    field.is_single_checkbox = True
                elif q.field_type == 'number':
                    field = forms.FloatField(**f_kwargs, widget=forms.NumberInput(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}))
                elif q.field_type == 'phone':
                    # Accept international loosely, e.g. +1 555-1234
                    f_kwargs['widget'] = forms.TextInput(attrs={'type': 'tel', 'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'})
                    field = forms.CharField(**f_kwargs)
                elif q.field_type == 'email':
                    field = forms.EmailField(**f_kwargs, widget=forms.EmailInput(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}))
                elif q.field_type == 'url':
                    field = forms.URLField(**f_kwargs, widget=forms.URLInput(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}))
                else:
                    field = forms.CharField(**f_kwargs, widget=forms.TextInput(attrs={'class': 'block w-full rounded-md border-surface-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm'}))
    ...
    def clean_timestamp_token(self):
        ...
        try:
            ...
            if datetime.now(timezone.utc).timestamp() - timestamp < 2.0:
