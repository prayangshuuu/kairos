...
def sync_user_entitlements_and_grandfathering(user):
    ...
    if max_event_types is None or max_event_types == float('inf'):
        ...
        if hidden_count > 0:
            logger.info(f"Grandfathered {hidden_count} extra event types for user {user.email} (now hidden/inactive)")


@transaction.atomic
def record_subscription_event(subscription: Subscription, event_type: str, new_status: str, payload: dict = None) -> SubscriptionEvent:
...
def process_stripe_subscription_webhook(event_data: dict) -> None:
    ...
    if event_type in ('customer.subscription.created', 'customer.subscription.updated'):
        ...
        if customer_id:
            ...
            if sub:
                record_subscription_event(sub, 'invoice.payment_failed', Subscription.STATUS_PAST_DUE, payload=event_data)
        ...
        if customer_id:
            ...
            if sub:
                ...
                record_subscription_event(sub, 'customer.subscription.deleted', Subscription.STATUS_EXPIRED, payload=event_data)
...
def check_and_process_paystation_renewals_and_grace():
    ...
    """
    now = timezone.now()
    bdt_subs = Subscription.objects.filter(provider=Subscription.PROVIDER_PAYSTATION, plan_code='pro')

    for sub in bdt_subs:
        if not sub.current_period_end:
            continue

        days_until_end = (sub.current_period_end - now).days
        ...
        if sub.status == Subscription.STATUS_ACTIVE:
            if days_past_end > 0 and days_past_end <= 7:
                ...
                record_subscription_event(sub, 'paystation_entered_grace_period', Subscription.STATUS_GRACE_PERIOD)
            ...
            if days_past_end > 7:
                ...
                sub.plan_code = 'free'
                sub.save(update_fields=['plan_code'])
                record_subscription_event(sub, 'paystation_grace_period_expired', Subscription.STATUS_EXPIRED)
