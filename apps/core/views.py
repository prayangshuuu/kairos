...
def resend_webhook(request):
    ...
    if event_type in ["email.bounced", "email.complained"]:
        ...
        if to_email:
            from apps.core.models import BouncedEmail, EmailEvent

            # Log the event
            EmailEvent.objects.create(
                recipient=to_email, event_type=event_type, message_id=message_id, payload=payload
            )
            ...
            BouncedEmail.objects.get_or_create(
                email=to_email, defaults={"reason": f"Resend event: {event_type}"}
            )
