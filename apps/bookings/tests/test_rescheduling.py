...
from apps.bookings.services import create_booking, cancel_booking, reschedule_booking, SlotUnavailable, AlreadyCancelled
...
def test_successful_reschedule(mock_is_slot_available):
    ...
    b1 = create_booking(
        event_type=event, start_at=start_at, invitee_name="Test", invitee_email="t@t.com",
        invitee_timezone="UTC", answers={}, now=now
    )
    
    new_start = start_at + timedelta(days=1)
    
    b2 = reschedule_booking(
        booking=b1, new_start_at=new_start, rescheduled_by="invitee", reason="Needed change", now=now
    )
...
def test_reschedule_overlapping_own_slot():
    ...
    schedule = Schedule.objects.create(user=user, name="Default", timezone="UTC")
    event = EventType.objects.create(owner=user, schedule=schedule, title="Test", slug="test", duration_minutes=60)
...
def test_reschedule_rollback_on_conflict():
    ...
    with patch('apps.bookings.services.is_slot_available', return_value=True):
        b1 = create_booking(event_type=event, start_at=slot1, invitee_name="T1", invitee_email="1@t.com", invitee_timezone="UTC", answers={}, now=now)
        b2 = create_booking(event_type=event, start_at=slot2, invitee_name="T2", invitee_email="2@t.com", invitee_timezone="UTC", answers={}, now=now)
        
        # b1 tries to reschedule into slot2 (which is occupied)
        with pytest.raises(SlotUnavailable):
            reschedule_booking(booking=b1, new_start_at=slot2, rescheduled_by="invitee", reason="", now=now)
...
def test_reschedule_chain_property(mock_is_slot_available):
    ...
    b1 = create_booking(event_type=event, start_at=now + timedelta(days=1), invitee_name="T", invitee_email="t@t.com", invitee_timezone="UTC", answers={}, now=now)
    b2 = reschedule_booking(booking=b1, new_start_at=now + timedelta(days=2), rescheduled_by="invitee", now=now)
    b3 = reschedule_booking(booking=b2, new_start_at=now + timedelta(days=3), rescheduled_by="invitee", now=now)
